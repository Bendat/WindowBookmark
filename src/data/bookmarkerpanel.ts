...
slf.port.on("closetab", (tabArr:string[], title)=>{
...
slf.port.on("update", (tabArr:string[], title)=>{
...
slf.port.on("bookmarked", (stored: Object)=>{
...
function removeBookmark(stored: string[]){
...
function listBookmarks(stored: string[]){
...
function bookmarkClickHandler(e){
    const wndw = (e.currentTarget as HTMLElement).innerText;
    slf.port.emit("openBookmark", wndw);
}
function swapId(oldId: string, newId: string, title: string): void{
...
function addListEntry(windId: string, title: string): void{
...
function makeId(windId: string): string{
    return "window"+windId;
} ;

function windowClickHandler(e: Event){
    toggleForm();
    const wndw = getWindowName((e.currentTarget as HTMLElement).id);
